...
# #### 3. BLUE 估计量
#
# **定义**：BLUE (Best Linear Unbiased Estimator，最佳线性无偏估计) 指的是在所有满足：
#
# - **线性**（是 $y$ 的线性组合），
# - **无偏**（期望等于被估参数）的估计量中，
# - 具有**最小方差**的估计量。
...
# %% [markdown] jp-MarkdownHeadingCollapsed=true slideshow={"slide_type": "subslide"} tags=["sub-slide"]
# #### (2) 证明 $SS_{H_e} - SS_e \sim \sigma^2 \chi^2_{m, \delta}$ · 题目
#
# **题目**
#
# 2. $SS_{H_e} - SS_e = (H\hat{\beta})^\top \left( H (X^\top X)^+ H^\top \right)^{-1} (H\hat{\beta}) \sim \sigma^2 \chi^2_{m, \delta}$, 其中:
